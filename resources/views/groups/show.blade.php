...
@section('content')
                ...
                <div
                    ...
                    <form action="{{ route('posts.store') }}" method="POST">
                        ...
                        <div style="text-align: right; margin-top: 10px;">
                            <button type="submit"
                                style="background: #e51f28; color: #fff; border: none; padding: 8px 16px; border-radius: 6px; font-weight: bold; cursor: pointer;">Đăng
                                bài</button>
                        </div>
                    </form>
                </div>
                        ...
                        <div class="card-header d-flex justify-content-between align-items-center"
                            ...
                            <div class="d-flex align-items-center">
                                <a href="{{ route('profile.show', $post->user->username) }}"
                                    onclick="event.stopPropagation()" class="post-user-link">
                                ...
                                @if (auth()->check() && auth()->id() !== $post->user_id)
                                    <form action="{{ route('follow.toggle', $post->user_id) }}" method="POST"
                                        class="ms-5 m-0" style="margin-left: 20px; display: inline-block;">
                                        @csrf
                                        @php
                                ...
                                @if ($group->creator_id === auth()->id() && $post->user_id !== auth()->id())
                                    @if (in_array($post->user_id, $approvedMemberIds))
                                        <form action="{{ route('groups.kick', [$group->slug, $post->user_id]) }}"
                                            method="POST" class="ms-3 m-0 prevent-post-modal no-post-modal"
                            ...
                            @if (auth()->id() == $post->user_id)
                                <div class="dropdown prevent-post-modal no-post-modal">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        ...
                                        <li>
                                            <form action="{{ route('post.toggleComment', $post->id) }}" method="POST"
                                                class="m-0">
                                                @csrf
                                        ...
                                        <div class="row g-1">
                                            @foreach ($media->take(4) as $index => $item)
                                                <div class="col-3 position-relative">
                                                    <img src="{{ $getImageUrl($item->media_url) }}" alt="post-image"
                        ...
                        <div class="card-actions d-flex justify-content-between align-items-center border-top pt-3 px-1 prevent-post-modal no-post-modal"
                            ...
                            <div class="d-flex align-items-center gap-3"
                                ...
                                <form action="{{ route('posts.save', $post->id) }}" method="POST"
                                    ...
                                    @csrf
                                    <button type="submit"
                                        class="btn-action save-btn {{ $isSaved ? 'saved text-warning' : '' }}"
                            ...
                            <button type="button"
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 640 640"
                                    width="22" height="22" opacity="0.7">
                                    <path
    ...
    {{-- ✨ ĐÃ THÊM: TOÀN BỘ POPUP MODAL CHỈNH SỬA THÔNG TIN NHÓM & UPDATE COVER --}}
    @if ($group->creator_id === auth()->id())
