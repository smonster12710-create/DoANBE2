        ...
        <div class="profile-container">
                    ...
                    <div class="list-header"
                        ...
                        <div class="list-search-box">
                            <input type="text" id="userSearchInput" placeholder="Tìm kiếm..."
                                style="border-radius: 20px; padding: 6px 15px; border: 1px solid #e4e6eb;">
                        </div>
                    </div>
                            ...
                            @foreach ($users as $item)
                                <a href="{{ route('profile.show', $item->username) }}" class="user-list-item">
                                    <div class="avatar-online-wrap">
                                        <img src="{{ $item->avatar_url ? asset($item->avatar_url) : asset('img/user/user.jpg') }}"
                                            alt="avatar">
                                    ...
                                    @foreach ($friendRequests as $item)
                                        <div class="user-list-item"
                                            ...
                                            <div class="request-actions-inline">
                                                <form method="POST"
                                                    action="{{ route('friend.accept', $item->username) }}">
                                                    @csrf
                                                    <input type="hidden" name="expected_status" value="requested">
                                                    <button type="submit" class="profile-action-btn primary">Chấp
                                                        nhận</button>
                                                </form>
                                                <form method="POST"
                                                    action="{{ route('friend.remove', $item->username) }}">
                                                    @csrf
                                                    <input type="hidden" name="expected_status" value="requested">
                                                    <button type="submit" class="profile-action-btn secondary">Từ
                                                        chối</button>
                                                </form>
                                            </div>
                                        </div>
            ...
            <script>
                ...
                searchInput.addEventListener('input', function() {
                    ...
                    const targetSelector = isFollowPage ? '#userListGrid .user-list-item' :
                        '#all-friends-tab .user-list-item';
