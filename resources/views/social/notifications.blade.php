    ...
    <div class="card border-0 shadow-sm rounded-3 p-2 mx-auto" style="max-width: 600px;">
        ...
        @forelse($notifications as $noti)
            ...
            if ($noti->type == 'comment') {
                ...
                $badgeColor = 'bg-info text-white'; // Màu xanh dương nhạt
                $icon = '<i class="fas fa-user-plus"></i>';
                $actionText = 'đã bắt đầu theo dõi bạn.';
            } elseif ($noti->type == 'friend_request') {
                $badgeColor = 'bg-warning text-dark'; // Màu vàng (để text dark cho dễ nhìn icon)
            ...
            $timeClass = $noti->is_read == 0 ? 'text-primary fw-bold' : 'text-muted fw-medium';
                    @endphp
                        ...
                        <div class="noti-item d-flex align-items-center p-2 rounded-3 mb-1 transition-fast {{ $bgClass }}"
                            ...
                            <a onclick="removeUnreadUI(this)" data-href="{{ route('notifications.read', $noti->id) }}" href="javascript:void(0);"
                            ...
                            <div class="d-flex align-items-center gap-2 flex-shrink-0">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0" style="font-size: 14px;">
                                        ...
                                        @if($noti->is_read)
                                            <li>
                                                <form action="{{ route('notifications.unread', $noti->id) }}" method="POST" class="m-0 ajax-noti-form"
                                                    data-type="unread">
                                                    @csrf
                                        ...
                                        {{-- Nút Xóa thông báo --}}
                                        <li>
                                            <form action="{{ route('notifications.destroy', $noti->id) }}" method="POST" class="m-0 ajax-noti-form"
                                                data-type="delete">
                                                @csrf
                                                @method('DELETE')
