        ...
        <div class="row justify-content-center">
            <div class="col-12 col-md-8 col-lg-6">
                ...
                <div class="detail-card card post-clickable" data-post-id="{{ $post->id }}">
                    ...
                    {{-- HEADER: ĐÃ NGỤY TRANG CHUẨN --}}
                    <div
                        class="detail-header card-header d-flex justify-content-between align-items-center bg-transparent border-0 p-3">
                        <div class="d-flex align-items-center">
                            @if($post->is_anonymous)
                                <img class="avatar" src="{{ asset('img/default-anonymous.png') }}" alt="Ẩn danh"
                                    style="width:40px; height:40px; border-radius:50%; object-fit:cover;">
                                <div class="detail-info ms-2">
                                    <strong class="name d-block text-dark">Người dùng ẩn danh 🕵️</strong>
                            ...
                            @else
                                ...
                                <div class="detail-info ms-2">
                                    <a href="{{ route('profile.show', $post->user->username) }}"
                                        ...
                                        <strong
                                            class="detail-name d-block text-dark">{{ $post->user->fullname ?? 'Người dùng' }}</strong>
                                        @if($post->parent_id)
                                ...
                                @if (auth()->check() && auth()->id() !== $post->user_id)
                                    <form action="{{ route('follow.toggle', $post->user_id) }}" method="POST" class="ms-5 m-0">
                                        ...
                                        @endphp
                                        <button type="submit"
                                            class="btn p-0 border-0 fw-bold {{ $isFollowing ? 'btn-following' : 'btn-follow' }}"
                        ...
                        {{-- NÚT 3 CHẤM (CHỈ CHỦ BÀI MỚI THẤY) --}}
                        @if (auth()->id() == $post->user_id)
                            <div class="dropdown">
                                <button class="more-btn border-0 bg-transparent" type="button"
                    ...
                    <div class="card-body p-3 pt-0">
                        ...
                        @if($post->parent_id && $post->parent)
                            <div class="original-post-block border rounded p-3 bg-light mb-3 mx-1">
                                ...
                                {{-- VÁ LỖ HỔNG RÒ RỈ DANH TÍNH BÀI GỐC Ở ĐÂY NÈ PRO --}}
                                <div class="d-flex align-items-center mb-2">
                                    @if($post->parent->is_anonymous)
                                        <img src="{{ asset('img/default-anonymous.png') }}"
                                            style="width:30px; height:30px; border-radius:50%; object-fit:cover;" alt="Ẩn danh">
                                ...
                                <div class="fb-gallery mb-3">
                                    @if ($count == 1)
                                        <div class="single-image">
                                            <img src="{{ $getImageUrl($media[0]->media_url) }}" alt="post-image" class="w-100 rounded"
                                                style="max-height: 600px; object-fit: contain; background: #f8f9fa;" loading="lazy"
                    ...
                    {{-- ACTIONS: LIKE, COMMENT, SAVE, PIN, SHARE --}}
                    <div class="detail-actions d-flex justify-content-between align-items-center border-top pt-3 px-3 pb-3">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            {{-- COMMENT --}}
                            <button type="button" class="btn-action border-0 bg-transparent p-0 d-flex align-items-center"
                            ...
                            <form action="{{ route('posts.save', $post->id) }}" method="POST" class="m-0 ajax-save-form">
                                @csrf
                                <button type="submit"
                                    class="btn-action save-btn {{ $isSaved ? 'saved text-warning' : '' }} border-0 bg-transparent p-0">
                            ...
                            @if(auth()->id() == $post->user_id)
                                <form action="{{ route('post.pin', $post->id) }}" method="POST" class="m-0 ajax-pin-form">
                                    @csrf
                                    <button type="submit"
                                        class="btn-action pin-btn {{ $post->is_pinned ? 'is-pinned text-primary' : '' }} border-0 bg-transparent p-0">
